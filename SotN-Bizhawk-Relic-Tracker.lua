...
local locations = {
    {
        ...
        path = "images/PowerOfMist.png",
        status = false,
        mapTiles = {{address = 0x06BC0B, values = {1}}}, -- doesnt trigger, but works on reload
        mapX = 250,
        mapY = 36
    }, {
        name = "Cube of Zoe",
        ...
        mapTiles = {
            ...
            {address = 0x06BC2B, values = {1}} ---doesnt trigger, but works on reload
        },
        mapX = 250,
        mapY = 52
    }, {
...
while true do
    if alucardModeStarted then
        ...
        if mainmemory.read_u8(alucardRoomsCountAddress) > alucardRooms then
            alucardRooms = mainmemory.read_u8(alucardRoomsCountAddress)
            detectLocations()
        end
